...
**`nextflow.config` only parses under the legacy (v1) config parser.** Nextflow 26 makes its strict config parser the default. The secret env-injection block that used to break it is **gone** (replaced by §4.9's secret directive + AWS bridge), so the **only** remaining v2 blocker is the `def check_max(...)` function definition (`nextflow.config:~145`) — the v2 parser rejects function definitions in config. The stub harness sidesteps this by exporting `NXF_SYNTAX_PARSER=v1`, so it verifies the pipeline runs on the 26 *engine* — but real users on 26 with default settings will hit a `Config parsing failed` error. **Follow-up not yet done:** migrate `check_max` out of config (nf-core's modern idiom is `process.resourceLimits`), then drop the `NXF_SYNTAX_PARSER=v1` export from `tests/run-stub-tests.sh`. Until then, don't add v1-only config idioms (chained assignments, top-level `if`/`def`).
