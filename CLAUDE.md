...
**`-Djava.aws.headless=true` is a typo.** The real JVM property is `java.awt.headless`. The flag is currently silently ignored. It appears in every Java-invoking module (`filter_pin.nf`, `panorama.nf`, `limelight_xml_convert_{combined,separate}.nf`, `limelight_upload_{combined,separate}.nf`). If you fix it, fix it in all of them in one change and confirm with the user first — "fixing" it may surface a previously-hidden headless AWT issue at runtime.
