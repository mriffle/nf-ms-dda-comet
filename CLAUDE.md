...
- **Every user-settable param should be in the schema, but an unknown param only warns — it no longer fails the run.** `validation.logging.unrecognisedParams = 'warn'` in `nextflow.config` (was `'error'`; relaxed because stray/carried-forward params like an old `--max_cpus` from a copied config were aborting runs). The tradeoff: a typo'd `--flag` is now silently ignored rather than caught. **Type validation of *known* params stays strict** — that's the `validateParameters()` core, independent of this flag, so a known param with a bad value still fails. Still add/rename a param here in the same change as the code (the warning is noise otherwise, and it keeps `paramsSummaryLog` accurate). This includes the `images` container map (modelled as a nested object in the `container_image_options` group) — we describe it in the schema rather than ignore-listing it.
